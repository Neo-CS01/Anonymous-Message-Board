    ...
    .delete(async function (req, res) {
      try {
        ...
        const index = threads.findIndex(thread =>
          thread._id === thread_id && thread.delete_password === delete_password
        );
    ...
    .delete(async function (req, res) {
      try {
        ...
        const reply = thread.replies.find(reply =>
          reply._id === reply_id && reply.delete_password === delete_password.trim()
        );

        if (!reply) {
          console.timeEnd("[DELETE] - /api/replies/:board");
          return res.send("Reply not found or incorrect password");
        }
